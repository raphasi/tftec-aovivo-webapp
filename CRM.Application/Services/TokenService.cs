...
namespace CRM.Application.Services
{
    public class TokenService : ITokenService
    {
        ...
        public async Task<string> GenerateAccessTokenAsync(IConfiguration configuration)
        {
        ...
        public string GenerateToken(ClaimsPrincipal user, IConfiguration configuration)
        {
            // Aqui você pode configurar a geração do token JWT
            var claims = new[]
            {
            ...
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(configuration["AzureB2C:Key"]));
            var creds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

            var token = new JwtSecurityToken(
                issuer: configuration["AzureB2C:Issuer"],
                audience: configuration["AzureB2C:Audience"],
                claims: claims,
